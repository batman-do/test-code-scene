...
class ComponentMeta(type):
    def __call__(cls, *args, **kwargs):
        ...
        if not hasattr(instance, "__canals_output__"):
            ...
            instance.__canals_output__ = deepcopy(
                getattr(instance.run, "_output_types_cache", {})
            )
        ...
        for param in list(run_signature.parameters)[
            1:
            ...
            if run_signature.parameters[param].kind not in (
                ...
                if run_signature.parameters[param].default != inspect.Parameter.empty:
                    socket_kwargs["default_value"] = run_signature.parameters[
                        param
                    ].default
...
class _Component:
    ...
    def set_input_type(self, instance, name: str, type: Any, default: Any = _empty):
        ...
        if not hasattr(instance, "__canals_input__"):
            instance.__canals_input__ = {}
        instance.__canals_input__[name] = InputSocket(
            name=name, type=type, default_value=default
        )
    ...
    def _component(self, class_):
        ...
        if not hasattr(class_, "run"):
            raise ComponentError(
                f"{class_.__name__} must have a 'run()' method. See the docs for more information."
            )

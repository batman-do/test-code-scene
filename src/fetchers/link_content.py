...
from .dataclasses import ByteStream
...
class LinkContentFetcher:
    ...
    def __init__(
        ...
        self.handlers: Dict[str, Callable[[Response], ByteStream]] = defaultdict(
            lambda: text_content_handler
        )
    ...
    def _fetch_with_exception_suppression(
        self, url: str
    ...
    def _switch_user_agent(self, retry_state: RetryCallState) -> None:
        ...
        """
        self.current_user_agent_idx = (self.current_user_agent_idx + 1) % len(
            self.user_agents
        )
        logger.debug(
            "Switched user agent to %s", self.user_agents[self.current_user_agent_idx]
        )

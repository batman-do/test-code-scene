...
def display_code_stream(stream):
    ...
    try:
        ...
        console = Console(
            record=True
        )  # Create a Console object that records print calls

        for output in stream:
            output_code = output.token.text
            output_code = output_code.replace("```", "") if output_code else output_code
            output_code = (
                output_code.replace("</s>", "") if output_code else output_code
            )
            if output_code == "\n":
                highlighted_code = Syntax(
                    code, "python", theme="monokai", line_numbers=False, word_wrap=True
                )

...
class AsyncRateLimiter(RateLimiter):
    ...
    async def __aexit__(self, exc_type, exc_val, exc_tb):
        await RateLimiter.__exit__(self, exc_type, exc_val, exc_tb)


# Tokenbucket
class TokenBucketChat(TokenBucket):
    def __init__(self, replenish_time: int, max_tokens: int) -> None:
        super().__init__(replenish_time, max_tokens)
        self.remain_time: float = datetime.now(timezone.utc).timestamp()


class RuntimeStorageChat(RuntimeStorage):
    def replenish(self, bucket: TokenBucketChat):
        ...
        if (current_time < bucket.last_replenished) or (
            ...
            bucket.remain_time = bucket.replenish_time - (
                current_time - bucket.last_replenished
            )
            return
